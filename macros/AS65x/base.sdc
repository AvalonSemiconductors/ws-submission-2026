...
set_input_delay $input_delay_value -clock $clocks -clock_fall $all_inputs_wo_clk_rst
set_output_delay $output_delay_value -clock $clocks -clock_fall [all_outputs]
...
set_driving_cell \
    -lib_cell gf180mcu_ocd_io__in_c \
    -pin PAD \
    $clk_input

set_driving_cell \
    -lib_cell gf180mcu_ocd_io__bi_24t \
    -pin PAD \
    [get_ports {io_in[*]}]
...
set_false_path -from [get_port select_6502]
set_false_path -through [get_ports {*pla_dlyb_*}]

#To accomodate slower machines the cpu may be used in, provide some additional hold time (at the cost of max clock rate)
set_output_delay -min 2 -clock $clocks -clock_fall [get_ports {io_out[*]}]
