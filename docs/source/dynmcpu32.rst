...
MCPU32 Dynamic DFF Test
=======================

This layout uses exclusively dynamic DFFs for all registers. These offer a performance advantage over static DFFs at the cost of requiring a certain minimum clock rate to not loose their state. What this clock rate is for this specific process and SCL is to be determined using this layout.

This layout is a minimal CPU architecture with only 4 instructions, based off of `MCPU <https://github.com/cpldcpu/MCPU>`__, but extended to a 32-bit word-length.
...
.. list-table:: Pad description
    :name: dynmcpu32-pad-description
    :header-rows: 1
    ...
    * - ``bidir[0]``
      - ``GPIO[8]``
      - IO
      - Bi-directional General-Purpose IO port
    * - ``bidir[1]``
      - ``GPIO[7]``
      - IO
      - Bi-directional General-Purpose IO port
...
.. wavedrom::
    ...
    {name: 'A[15:0]', wave: 'x5.6.7.x.6.7.x', data: 'PC[29:16] PC[15:0] PC[15:0]+1 addr[15:0] addr[15:0]+1'},
